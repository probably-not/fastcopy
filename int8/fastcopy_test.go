...
func Test_FastCopyInt8_Simple(t *testing.T) {
	t.Parallel()
...
func Benchmark_FastCopyInt8_Simple(b *testing.B) {
	for _, tC := range testCases {
...
func Benchmark_FastCopyInt8_Complex(b *testing.B) {
	for _, tC := range testCases {
		b.Run(tC.desc, func(sub *testing.B) {
			original := make([]int8, tC.size)
			for i := 0; i < len(original); i++ {
			...
			sub.ReportAllocs()
			for i := 0; i < sub.N; i++ {
				CopyInt8Slice(cps[i&127], original)
			}
		})
	}
}
