...
func Test_FastCopyInt16_Simple(t *testing.T) {
	...
	for _, tC := range testCases {
		t.Run(tC.desc, func(sub *testing.T) {
			...
			cp := make([]int16, len(original))
			CopyInt16Slice(cp, original)
...
func Benchmark_FastCopyInt16_Simple(b *testing.B) {
...
func Benchmark_FastCopyInt16_Complex(b *testing.B) {
	for _, tC := range benchCases {
		b.Run(tC.desc, func(sub *testing.B) {
			original := make([]int16, tC.size)
			for i := 0; i < len(original); i++ {
			...
			var cps [128][]int16
			for i := range cps {
				cps[i] = make([]int16, tC.size)
			}

			sub.ResetTimer()
			sub.ReportAllocs()
			for i := 0; i < sub.N; i++ {
				CopyInt16Slice(cps[i&127], original)
			}
		})
	}
}

func Benchmark_CopyInt16_Simple(b *testing.B) {
	for _, tC := range benchCases {
		b.Run(tC.desc, func(sub *testing.B) {
			original := make([]int16, tC.size)
			for i := 0; i < len(original); i++ {
...
func Benchmark_CopyInt16_Complex(b *testing.B) {
	for _, tC := range benchCases {
		b.Run(tC.desc, func(sub *testing.B) {
			original := make([]int16, tC.size)
			for i := 0; i < len(original); i++ {
