...
func Test_FastCopyUint64_Simple(t *testing.T) {
	t.Parallel()
	...
	for _, tC := range testCases {
		t.Run(tC.desc, func(sub *testing.T) {
			...
			cp := make([]uint64, len(original))
			CopyUint64Slice(cp, original)
...
func Benchmark_FastCopyUint64_Simple(b *testing.B) {
	for _, tC := range testCases {
...
func Benchmark_FastCopyUint64_Complex(b *testing.B) {
	for _, tC := range testCases {
		b.Run(tC.desc, func(sub *testing.B) {
			original := make([]uint64, tC.size)
			for i := 0; i < len(original); i++ {
			...
			var cps [128][]uint64
			for i := range cps {
				cps[i] = make([]uint64, tC.size)
			}

			sub.ResetTimer()
			sub.ReportAllocs()
			for i := 0; i < sub.N; i++ {
				CopyUint64Slice(cps[i&127], original)
			}
		})
	}
}

func Benchmark_CopyUint64_Simple(b *testing.B) {
	for _, tC := range testCases {
		b.Run(tC.desc, func(sub *testing.B) {
			original := make([]uint64, tC.size)
			for i := 0; i < len(original); i++ {
...
func Benchmark_CopyUint64_Complex(b *testing.B) {
	for _, tC := range testCases {
		b.Run(tC.desc, func(sub *testing.B) {
			original := make([]uint64, tC.size)
			for i := 0; i < len(original); i++ {
